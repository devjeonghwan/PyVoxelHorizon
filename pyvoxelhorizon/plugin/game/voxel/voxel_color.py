...
VOXEL_COLOR_PALETTE_COLOR_COUNT = 32
VOXEL_COLOR_PALETTE = [
...
def find_similar_voxel_color(red: int, green: int, blue: int) -> VoxelColor:
    key = "{0}_{1}_{2}".format(red, green, blue)

    if key not in VOXEL_COLOR_RGB_CACHE:
        find_distance = 255 * 255 * 255
        find_color = None

        for index in range(VOXEL_COLOR_PALETTE_COLOR_COUNT):
            color = VOXEL_COLOR_PALETTE[index]
